...
1. `azfls/model.py` 直驱 `mlx_lm.generate_step`，只在最后 decode 一次。
   ...
   构造要遍历 15 万条词表，不缓存）。
2. `azfls/decide.py` 的决策契约换成精简 JSON `{"f": ..., "r": [...]}`，
   旧的长键仍然接受（向后兼容）。
...
from azfls.decide import (  # noqa: E402 - 先修好 sys.path 再导入
    FUNCTION_ID,
    Candidates,
    Decision,
    DecisionError,
...
from azfls.gate import Gate  # noqa: E402
from azfls.model import DEFAULT_MODEL, MLXEngine, clean_body  # noqa: E402
...
def end_to_end(engine: MLXEngine, candidates: Candidates, expected: Decision) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix="azfls-speedup-") as scratch:
