...
`bench/candidate_probe.py` 记下的现状是 2/5；而 `codejev/decide.py` 的选择路线
实测过“系统提示里少一行工作示例”会把 15/15 打成 3/15（模型整个省掉 `f` 键）。
所以本脚本只问一个问题：候选页的 2/5 有多少来自提示写法，多少来自模型能力。

三个变体（**措辞在跑之前写死**，跑完不看结果换措辞）：

- V0：现状，逐字使用 `codejev.candidate.build_selection_messages` 的产物；
...
严格解析器（`codejev.candidate.parse_choice`）一个字都没改。本脚本不写盘、不改生产代码，
产出只有 stdout 上的证据。
...
from codejev.candidate import (  # noqa: E402 - 先修好 sys.path 再导入
    _SYSTEM_PROMPT,
    CandidateError,
    CodeTask,
...
from codejev.model import MLXEngine, Stats  # noqa: E402
