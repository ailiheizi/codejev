...
1. 顺序调用（现有生产路径 azfls.model.MLXEngine，一次一个请求）与批处理
...
4. 答案正确性：每条答案都走项目现有的运行时检查——取出所选 id，用
   azfls.decide.assemble 组装，真的 exec 并调用函数，核对过滤结果与返回字段
   恰好等于要求（不是看代码像不像）。
...
本脚本只读现有模块，不修改任何 azfls 代码；模型只加载一次。
"""
...
if str(ROOT) not in sys.path:  # 直接以脚本方式运行时也能 import azfls
...
class Request:
    ...
    prompt_ids: list[int]
    candidates: object  # azfls.decide.Candidates
    expected: object  # azfls.decide.Decision
...
def encode_prompt(tokenizer, messages: list[dict[str, str]]) -> list[int]:
    """与 azfls.model.MLXEngine.generate 里的编码规则逐字一致（含 add_special_tokens）。"""
...
def check_quality(decide, request: Request, text: str) -> Quality:
    ...
    模型回的是决策 JSON（候选 id），代码由 azfls.decide.assemble 落成；所以
    "答案正确"分三步查：
...
def run_batched(engine, tokenizer, requests: list[Request], max_tokens: int) -> BatchOutcome:
    ...
    from azfls.model import clean_body
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--model", default=None, help="模型目录（默认用 azfls.model.DEFAULT_MODEL）")
    ...
    decide = import_project_module("azfls.decide")
    model_mod = import_project_module("azfls.model")
