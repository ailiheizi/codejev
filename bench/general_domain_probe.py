...
沿用现有协议，不改 azfls/ 下任何文件：
- 候选页仍是 `azfls.candidate.CandidatePage` / `CodeCandidate`；
- 选择仍是 `choose_candidate` + `parse_choice` 严格解析；
- 物化仍是 `materialize`；
- 执行器仍是 `azfls.api_engine.OpenAICompatibleEngine`，配置照抄
...
from azfls.api_engine import APIConfig, APIEngineError, OpenAICompatibleEngine
...
def prompt_chars(task: CodeTask, page: CandidatePage) -> int:
    from azfls.candidate import build_selection_messages
...
def part_b_other_files() -> None:
    ...
    for path in sorted((*REPO_ROOT.glob("azfls/*.py"), *REPO_ROOT.glob("bench/*.py"), *REPO_ROOT.glob("tests/*.py"))):
        enumeration = de.enumerate_source(path, "python312")
        for entry in enumeration.functions:
...
def part_b_materialize_limits(source_module: ModuleType, source: Path) -> None:
    ...
    decorated = []
    for path in sorted(REPO_ROOT.glob("azfls/*.py")):
        enumeration = de.enumerate_source(path, "python312")
        for entry in enumeration.functions:
            if entry.decorators:
                decorated.append((path, entry))
    print(f"  3) 带装饰器的函数/方法：azfls 包内 {len(decorated)} 个；源码段不含装饰器行")
...
def part_b_not_verified() -> None:
    ...
    for line in (
        ...
        "只有 azfls/candidate.py 一个主域源文件；B3 的其它文件只做静态普查，没跑任务。",
...
def main(argv: list[str] | None = None) -> int:
    ...
    heading("az-fls 域无关候选页原型：第二个域 = Python 函数与变量")
    print(f"source={source.relative_to(REPO_ROOT)} sha256[:16]={digest} lines={len(source.read_text(encoding='utf-8').splitlines())}")
    print(f"python={sys.version.split()[0]}")
    print("protocol=azfls.candidate（CandidatePage/CodeCandidate/choose_candidate/materialize，未改动）")
    if args.no_api:
    ...
    decide = REPO_ROOT / "azfls" / "decide.py"
    other = [REPO_ROOT / "azfls" / "decide.py", REPO_ROOT / "azfls" / "api_engine.py"]
