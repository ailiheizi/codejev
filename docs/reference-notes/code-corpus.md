...
本次文档工作开始时，当前 `az-fls` 目录没有源代码。本次没有遍历用户整块磁盘，也没有声称用户其他本地项目缺少代码。已有公开仓库已经能提供几个很具体的起点。
...
来源：[ailiheizi/loom](https://github.com/ailiheizi/loom)。本次读取 archive 的 commit 前缀为 `c299faf`，本地快照目录为 `/tmp/az-fls-repo-review/ailiheizi__loom/src/ailiheizi-loom-c299faf/`。未在这次调查中解析完整 commit；下列链接固定到该已知前缀。
...
来源：[ailiheizi/universal-selector](https://github.com/ailiheizi/universal-selector)。本次读取 archive 的 commit 前缀为 `f014818`，本地快照目录为 `/tmp/az-fls-repo-review/ailiheizi__universal-selector/src/ailiheizi-universal-selector-f014818/`。未在这次调查中解析完整 commit。
